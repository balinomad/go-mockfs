...
func TestNewMockFile(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			file, err := mockfs.NewMockFile(tt.mapFile, "test.txt", mockfs.WithFileOverwrite())
			if tt.wantErr {
				assertAnyError(t, err, "NewMockFile()")
			...
			requireNoError(t, err, "NewMockFile()")
			if file == nil {
...
func TestMustNewMockFile_Panic(t *testing.T) {
	t.Parallel()
	requirePanic(t, func() { mockfs.MustNewMockFile(nil, "test.txt") }, "MustNewMockFile() nil mapFile")
}
...
func TestNewMockFile_Defaults(t *testing.T) {
	...
	file := mockfs.MustNewMockFile(mapFile, "test.txt", mockfs.WithFileOverwrite())
...
func TestNewDirHandler(t *testing.T) {
	...
	t.Run("pagination boundary overflow", func(t *testing.T) {
		...
		moreEntries := []fs.DirEntry{
			...
			mockfs.MustNewFileInfo("file3.txt", 5, 0o644, time.Now()),
		}
...
func TestMockFile_Write(t *testing.T) {
	...
	t.Run("overwrite mode", func(t *testing.T) {
		...
		file := mockfs.MustNewMockFile(mapFile, "test.txt")
	...
	t.Run("append mode", func(t *testing.T) {
		...
		file := mockfs.MustNewMockFile(mapFile, "test.txt", mockfs.WithFileAppend())
	...
	t.Run("readonly mode", func(t *testing.T) {
		...
		file := mockfs.MustNewMockFile(mapFile, "test.txt", mockfs.WithFileReadOnly())
	...
	t.Run("error injection", func(t *testing.T) {
		...
		file := mockfs.MustNewMockFile(mapFile, "test.txt", mockfs.WithFileErrorInjector(inj))
...
// TestMockFile_WriteAt_LatencyBeforeError verifies that WriteAt applies configured
// latency before returning the read-only permission error (F-C3 fix), consistent
...
func TestMockFile_Seek(t *testing.T) {
	...
	// "seek to position" and "seek to end" share this file and depend on sequential
...
func TestMockFile_ReadDir(t *testing.T) {
	...
	t.Run("valid directory", func(t *testing.T) {
		t.Parallel()
		entries := []fs.DirEntry{
			mockfs.MustNewFileInfo("file1.txt", 0, 0o644, time.Now()),
			mockfs.MustNewFileInfo("file2.txt", 0, 0o644, time.Now()),
		}
	...
	t.Run("nil handler respects error injection", func(t *testing.T) {
		t.Parallel()
		inj := mockfs.NewErrorInjector()
		inj.AddExact(mockfs.OpReadDir, "emptydir", mockfs.ErrPermission, mockfs.ErrorModeAlways, 0)
		dir := mockfs.NewMockDir("emptydir", nil, mockfs.WithFileErrorInjector(inj))

		_, err := dir.ReadDir(-1)
		assertError(t, err, mockfs.ErrPermission, "nil handler must honour error injection")
	})

	t.Run("nil handler respects latency", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("pagination", func(t *testing.T) {
		t.Parallel()
		entries := []fs.DirEntry{
			mockfs.MustNewFileInfo("file1", 0, 0o644, time.Now()),
			mockfs.MustNewFileInfo("file2", 0, 0o644, time.Now()),
			mockfs.MustNewFileInfo("file3", 0, 0o644, time.Now()),
			mockfs.MustNewFileInfo("file4", 0, 0o644, time.Now()),
		}
...
func TestMockFile_Stat(t *testing.T) {
	...
	t.Run("file info", func(t *testing.T) {
		...
		file := mockfs.MustNewMockFile(mapFile, "testdir/test.txt")
	...
	t.Run("basename extraction", func(t *testing.T) {
		...
		tests := []struct {
			...
			{name: "simple", path: "test.txt", wantName: "test.txt"},
			{name: "one level deep", path: "dir/test.txt", wantName: "test.txt"},
			{name: "multiple levels", path: "a/b/c/file.dat", wantName: "file.dat"},
			{name: "absolute path", path: "/absolute/path/file", wantName: "file"},
		}
	...
	t.Run("permissions", func(t *testing.T) {
		...
		for _, mode := range modes {
			t.Run(fmt.Sprintf("mode 0%o", mode), func(t *testing.T) {
				t.Parallel()
				mapFile := &fstest.MapFile{
					Data:    []byte("test"),
					Mode:    mode,
